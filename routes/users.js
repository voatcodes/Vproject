        ...
        bcrypt.compare(
          ...
          (error, matches) => {
            if (matches) {
              ...
              res.redirect(`/users/profile/${results[0].userID}`)
            } else {
              const user = {
...
router.get("/profile/:id", (req, res) => {
  ...
  connection.query(sql, [parseInt(req.params.id)], (error, results) => {
    const profile = {
...
router.get("/editprofile/:id", (req, res) => {
  ...
  connection.query(sql, [parseInt(req.params.id)], (error, results) => {
    ...
    res.render("users/editprofile", { profile: profile });
  });
});
...
router.post("/editprofile/:id", upload.single("photoURL"), (req, res) => {
  ...
  if (req.file) {
    ...
    let sql =
      "UPDATE user_profile  SET dob=?, fullname = ?, preferedusername=?, email = ?, pnumber = ?,  facebookURL = ?, twitterURL = ?, instagramURL = ?, photoURL = ?, cphotoURL = ?, datejoined=? WHERE userID = ?";
